...
$fetchReservedrooms = mysqli_query($conn, "SELECT room_number FROM reservation_masterfile WHERE (((checkoutdate >= '{$_POST['checkInDate']}' AND checkindate <= '{$_POST['checkInDate']}') OR (checkoutdate >='{$_POST['checkOutDate']}' AND checkindate <= '{$_POST['checkOutDate']}')) AND room_id = {$_POST['room_id']}) AND (status = 'Approved' OR status = 'Checkin')") or die(mysqli_error($conn));
$reservedroomSum = 0;
